...
type Props = { paper: Paper | null; form: PaperForm; sections: string[]; saving: boolean; duplicate: string; onChange: (form: PaperForm) => void; onClose: () => void; onSubmit: (event: FormEvent) => void; onDelete: () => void };

export default function PaperDrawer({ paper, form, sections, saving, duplicate, onChange, onClose, onSubmit, onDelete }: Props) {
  return <div className="drawer-shell" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="drawer" role="dialog" aria-modal="true" aria-labelledby="drawer-title">
    <div className="drawer-head"><div><p>{paper ? 'Paper details' : 'New paper'}</p><h2 id="drawer-title">{paper ? paper.title : 'Add to your library'}</h2></div><button className="icon-button" onClick={onClose} aria-label="Close">×</button></div>
    <form onSubmit={onSubmit}>
      ...
      <div className="drawer-actions">{paper && <button type="button" className="danger" onClick={onDelete}>Delete</button>}<span />{paper?.url && <a className="secondary" href={paper.url} target="_blank" rel="noreferrer">Open Paper ↗</a>}<button type="submit" className="primary" disabled={saving}>{saving?'Saving…':paper?'Save changes':'Add Paper'}</button></div>
    </form>
  </aside></div>;
}
