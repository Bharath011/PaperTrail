...
type Props = { papers: Paper[]; sections: string[]; researcherCount: number; view: string; open: boolean; onView: (view: string) => void; onClose: () => void };

export default function Sidebar({ papers, sections, researcherCount, view, open, onView, onClose }: Props) {
  ...
  return <>
    ...
    <aside className={`sidebar ${open ? "open" : ""}`}>
      <div className="brand"><div className="brand-mark">P</div><div><h1>PaperTrail</h1><p></p></div></div>
      <nav aria-label="Primary navigation">
        <p className="nav-label">Library</p>{item("all", "All Papers", count(), "▤")}
        <div className="collections">{sections.map((section) => item(`section:${section}`, section, section === "People to Follow" ? researcherCount : papers.filter((paper) => paper.section === section).length, "–"))}</div>
